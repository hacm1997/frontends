...
export default function PlansUpdate(props:any) {
    ...
    const confirmation = (index:number) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                props.deleteImagePlans(index);
            }
        });
    }

    useEffect(()=>{
        if(props.newPlansDev.length < 1){
            // console.log("hola xd")
            setNewData(props.newPlans)
        }else{
            setNewData(props.newPlansDev)
        }
    }, [props.newPlans])
        ...
        return(
            <div key={index} className="col-4">
                <div className={styles.gallery}>
                    ...
                    <img src={item.data.toString()} width={270} height={170} alt="apartamentos"/>
                </div><br/>
            </div>
        )
